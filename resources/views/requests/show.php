...
const formatCurrency = (amount) => {
    ...
    function renderCategorySection(category, items) {
        ...
        const iconMap = {
            ...
            'ดำเนินงาน': 'briefcase',
            'ลงทุน': 'building-2',
            'อุดหนุน': 'hand-helping',
            'รายจ่ายอื่น': 'more-horizontal'
        };
        let mainIcon = 'files';
        ...
        categoryWrapper.innerHTML = `
            <div class="flex items-start gap-4 mb-6 px-2 border-b border-slate-800/50 pb-4">
                <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-700 flex items-center justify-center shadow-lg shadow-indigo-500/20 ring-1 ring-white/10 shrink-0">
                    <i data-lucide="${mainIcon}" class="text-white w-6 h-6"></i>
                </div>
                <div>
        ...
        items.forEach(item => {
            ...
            if (item.level === 1 && item.is_header) {
                ...
                const secIconMap = {
                    'เงินเดือน': 'banknote',
                    'เงินประจำตำแหน่ง': 'briefcase',
                    'ค่าตอบแทนรายเดือน': 'coins',
                    'เงินเพิ่มพิเศษ': 'star',
                    'ค่าจ้างประจำ': 'user-check',
                    'ค่าตอบแทนพนักงานราชการ': 'user-cog',
                    'ค่าตอบแทน': 'credit-card',
                    'ค่าใช้สอย': 'wrench',
                    'วัสดุ': 'package'
                };
                    ...
                    <button type="button" class="w-full flex items-center justify-between p-4 transition-colors ${isExpanded ? 'bg-indigo-500/5' : 'hover:bg-slate-800/50'}" onclick="toggleSection(this)">
                        <div class="flex items-center gap-4">
                            <div class="p-2 rounded-lg transition-colors ${isExpanded ? 'bg-indigo-500/20 text-indigo-300' : 'bg-slate-800 text-slate-400'}">
                                <i data-lucide="${secIcon}" class="w-5 h-5"></i>
                            </div>
                            <div class="text-left">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <i data-lucide="chevron-down" class="section-toggle-icon text-slate-500 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}"></i>
                        </div>
                    </button>
                        ...
                        <div class="md:col-span-4 space-y-1.5">
                            ...
                            <div class="relative">
                                <input type="text" 
                                    ...
                                    data-field="remark"
                                >
                                <i data-lucide="pencil" class="w-4 h-4 absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-600"></i>
                            </div>
                        </div>
                    </div>
                `;
...
async function handleBlur(e) {
    ...
    saveIndicator.innerHTML = '<i data-lucide="loader-2" class="animate-spin w-4 h-4"></i> กำลังบันทึก...';
    ...
    try {
        ...
        if (data.success) {
            ...
            saveIndicator.innerHTML = '<i data-lucide="check-circle" class="w-4 h-4 text-emerald-400"></i> บันทึกแล้ว';
            lucide.createIcons({ root: saveIndicator });
        ...
        saveIndicator.classList.remove('animate-pulse');
        saveIndicator.innerHTML = `<i data-lucide="alert-triangle" class="w-4 h-4 text-red-500"></i> <span class="text-red-400 text-xs">${err.message || 'บันทึกไม่สำเร็จ'}</span>`;
        lucide.createIcons({ root: saveIndicator });
