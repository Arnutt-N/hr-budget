<div class="max-w-2xl mx-auto">
    <!-- Header -->
    <div class="mb-8">
        <a href="/budgets/disbursements" class="inline-flex items-center gap-2 text-slate-400 hover:text-slate-200 transition-colors mb-4">
            <i data-lucide="arrow-left" class="w-4 h-4"></i>
            กลับหน้ารายการ
        </a>
        <h1 class="text-2xl font-bold text-slate-100">สร้างรายการเบิกจ่ายใหม่</h1>
        <p class="text-slate-400 mt-1">กรอกข้อมูลเบื้องต้นเพื่อเริ่มบันทึกรายการ</p>
    </div>

    <!-- Form Card -->
    <div class="bg-slate-800/50 border border-slate-700 rounded-xl p-6 shadow-xl">
        <form action="/budgets/disbursements" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="fiscal_year" required
                            class="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-100 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
                        <?php foreach ($fiscalYears as $year): ?>
                            <option value="<?= $year ?>" <?= $year == date('Y')+543 ? 'selected' : '' ?>>
                                <?= $year ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    ...
                    <select name="month" required
                        ...
                        ?>
                            <option value="<?= $k ?>" <?= $k == $curMonth ? 'selected' : '' ?>>
                                <?= $v ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
