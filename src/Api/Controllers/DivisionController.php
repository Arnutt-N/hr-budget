...
final class DivisionController
{
    public function __construct(
        private readonly DivisionService $service = new DivisionService()
    ) {}
    ...
    public function create(): void
    {
        ...
        try {
            $dto = CreateDivisionDto::fromRequest();
            $errors = $dto->validate();
            if (!empty($errors)) {
            ...
            error_log("[DivisionController::create] {$e->getMessage()}");
    ...
    public function update(string $id): void
    {
        ...
        try {
            $dto = UpdateDivisionDto::fromRequest();
            $errors = $dto->validate();
            if (!empty($errors)) {
                ApiResponse::validationFailed($errors);
                return;
            ...
            error_log("[DivisionController::update] {$e->getMessage()}");
            ApiResponse::error('เกิดข้อผิดพลาดในระบบ', 500);
        }
    }
    ...
    public function delete(string $id): void
    {
        ...
        try {
            ...
            error_log("[DivisionController::delete] {$e->getMessage()}");
            ApiResponse::error('เกิดข้อผิดพลาดในระบบ', 500);
        }
    }
}
