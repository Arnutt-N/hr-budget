...
final class BudgetTargetController
{
    public function __construct(
        private readonly BudgetTargetService $service = new BudgetTargetService()
    ) {}
    ...
    public function create(): void
    {
        ...
        try {
            $dto = CreateBudgetTargetDto::fromRequest();
            $errors = $dto->validate();
            if (!empty($errors)) {
            ...
            error_log("[BudgetTargetController::create] {$e->getMessage()}");
    ...
    public function show(string $id): void
    {
        ...
        if ($item === null) {
            ApiResponse::notFound('ไม่พบเป้าหมาย');
            return;
        }
    ...
    public function update(string $id): void
    {
        ...
        try {
            $dto = UpdateBudgetTargetDto::fromRequest();
            $errors = $dto->validate();
            if (!empty($errors)) {
                ApiResponse::validationFailed($errors);
                return;
            ...
            if (!$ok) {
                ApiResponse::error('ไม่สามารถแก้ไขเป้าหมายได้', 422);
                return;
            }
            ...
            error_log("[BudgetTargetController::update] {$e->getMessage()}");
            ApiResponse::error('เกิดข้อผิดพลาดในระบบ', 500);
        }
    }
    ...
    public function delete(string $id): void
    {
        ...
        try {
            ...
            if (!$ok) {
                ApiResponse::error('ไม่สามารถลบเป้าหมายได้', 422);
                return;
            }
            ApiResponse::noContent();
        } catch (\Throwable $e) {
            error_log("[BudgetTargetController::delete] {$e->getMessage()}");
            ApiResponse::error('เกิดข้อผิดพลาดในระบบ', 500);
        }
    }
}
