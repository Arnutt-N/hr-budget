...
try {
    ...
    // Enable buffered query to avoid "unbuffered queries are active"
    $pdo->setAttribute(PDO::MYSQL_ATTR_USE_BUFFERED_QUERY, true);
    
    echo "<h1>Running Migrations (Embedded SQL)</h1>";
    
    // Check version and clear
...
CREATE TABLE IF NOT EXISTS activities (
    ...
    plan_id INT NULL COMMENT 'FK: plans.id',
    code VARCHAR(50),
...
UPDATE organizations SET region = 'regional' WHERE region = 'regional'; -- dummy update
SQL;
    ...
    foreach ($sqls as $key => $content) {
        ...
        echo "<p>Count: " . count($statements) . "</p>";
        ...
        foreach ($statements as $sql) {
            ...
            try {
                $pdo->exec($sql);
                echo "<div style='color:green'>Success: " . htmlspecialchars(substr($sql, 0, 50)) . "...</div>";
            } catch (PDOException $e) {
                // Ignore duplicates
                $msg = $e->getMessage();
                if (strpos($msg, '1050') !== false || strpos($msg, 'already exists') !== false || strpos($msg, 'Duplicate column') !== false || strpos($msg, '1060') !== false) {
                    echo "<div style='color:orange'>Skipped (Exists): " . htmlspecialchars(substr($sql, 0, 50)) . "...</div>";
                } else {
                    echo "<div style='color:red'>Error: " . htmlspecialchars($msg) . "</div>";
                }
            }
        }
    }
    
    // View update (complex, do separately)
