...
echo "<h1>Syncing Budget Structure (v2)</h1>";
...
try {
    ...
    // Add 'output' to ENUM
    $pdo->exec("ALTER TABLE budget_plans MODIFY COLUMN plan_type ENUM('strategic','roadmap','program','project','output','activity','sub_activity') NOT NULL DEFAULT 'program'");
    echo "<p>Updated plan_type ENUM.</p>";
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        // Cols: plan_type, code, name_th, fiscal_year, level
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (!$parentBpId) {
             echo "skip proj {$row['id']} no parent<br>";
             continue;
        }
        
        $bType = ($row['project_type'] == 'output') ? 'output' : 'project';
        // Cols: parent_id, plan_type, code, name_th, fiscal_year, level
