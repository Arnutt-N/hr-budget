...
echo "<h1>Syncing Budget Structure to Budget Plans</h1>";

try {
    $pdo->beginTransaction();

    // 1. Clear budget_plans (Optional? Or Update? Truncate is cleaner if we rebuild)
    // Warning: IDs will change. If disbursement_records link to budget_plans.id, this is destructive!
    // Check if disbursement_records has data.
    $cnt = $pdo->query("SELECT COUNT(*) FROM disbursement_records")->fetchColumn();
    if ($cnt > 0) {
        throw new Exception("Cannot truncate budget_plans because disbursement_records has data ($cnt rows). Sync logic needs to be smarter (Update/Insert).");
    }
    ...
    // cache: generic_id (plan/project/activity id) -> budget_plan_id
    $planMap = []; 
    $projectMap = [];
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (!$parentBpId) {
            echo "Warning: Project {$row['code']} has no parent plan.<br>";
            continue;
        }
        
        $bType = ($row['project_type'] == 'output') ? 'output' : 'project';
